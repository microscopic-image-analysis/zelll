...
pub struct SmoothDistanceField {
    inner: CellGrid<WrappedParticle<Atom>, 3, Angstrom>,
...
impl SmoothDistanceField {
    pub fn new(protein: &PointCloud, cutoff: Angstrom) -> Self {
        Self {
            inner: CellGrid::new(
                protein.points.iter().copied().map(WrappedParticle::from),
                cutoff,
            ),
            surface_radius: 1.05,
            k_force: 10.0,
    ...
    pub fn grid(&self) -> &CellGrid<WrappedParticle<Atom>, 3, Angstrom> {
        &self.inner
    }
}
